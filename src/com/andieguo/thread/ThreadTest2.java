...
/**
 * sub thread seq of 0, loop of 0 sub thread seq of 1, loop of 0 main thread seq
 * of 0, loop of 0 main thread seq of 1, loop of 0 main thread seq of 2, loop of
 * 0 main thread seq of 3, loop of 0
 * 
 * sub thread seq of 0, loop of 1 sub thread seq of 1, loop of 1 main thread seq
 * of 0, loop of 1 main thread seq of 1, loop of 1 main thread seq of 2, loop of
 * 1 main thread seq of 3, loop of 1
 * 
...
/**
 * 
 * 异常：java.lang.IllegalMonitorStateException 首先你要了解这个异常为什么会抛出，这个异常会在三种情况下抛出：
 * 1>当前线程不含有当前对象的锁资源的时候，调用obj.wait()方法; 2>当前线程不含有当前对象的锁资源的时候，调用obj.notify()方法。
 * 3>当前线程不含有当前对象的锁资源的时候，调用obj.notifyAll()方法。
 * 
 * 最新解释： main首先执行到synchronized main（0）,获得锁对象资源（就绪队列为空），bool =
 * true，执行this.wait（）方法 释放锁对象 进入等待区等待； 时间片轮转到sub执行synchronized
 * sub（0），获得锁对象资源（就绪队列为空），bool = false,顺利执行完2次循环，将bool =
 * false,并通知main进入就绪队列（此时sub仍拥有着锁对象），执行sub（2），进入等候队列，释放锁对象；
